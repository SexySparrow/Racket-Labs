...
(define (num->base n b)
  (if (= n 0) null (append (num->base (quotient n b) b) (list (modulo n b)))))
...
(define (rev L)
  (if(null? L) L (append (rev (cdr L)) (list(car L)))))
...
(define (palindromes-to-n n Bases)
  (iterate n Bases null))


(define (iterate n Bases L)
  (if(< n 0)
     L
     (if (all-palindromes? n Bases)
         (iterate (- n 1) Bases (append (list n) L))
         (iterate (- n 1) Bases L))))

(define (list-of-pali n Bases L)
...
(define (first-b-pal start b)
  (iteratenum start b (list 2 3 4 5 6 7 8 9 10)))

(define (iteratenum start b Bases)
  (if(is-palib start b Bases)
     start
     (iteratenum (+ start 1) b Bases)))


(define (is-palib n b Bases)
  (if(null? Bases)
     (if(= b 0)
        #t
        #f)
     ...
     #t
     (if  (equal? (num->base n (car Bases)) (rev (num->base n (car Bases))))
          (is-palib n (- b 1) (cdr Bases))
          (is-palib n b (cdr Bases))))))
...
(define (longest-palindrome n)
   (itercheck (num->base n 10) null))

  


(define (itercheck L rez)
  (if (null? L)
      rez
      (append rez (checkpal L (length L)))))

(define (checkpal L size)
  (if (= size 0)
      null
      (if(equal? (take L size) (rev (take L size)))
         (take L size)
         (checkpal L (- size 1)))))
