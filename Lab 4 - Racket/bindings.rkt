...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let*
      ([segm (get-line-segment)]
       [p1 (get-start-point segm)]
       [p2 (get-end-point segm)]
       [t1 (* (- (car p1) (car p2)) (- (car p1) (car p2)))]
       [t2 (* (- (cdr p1) (cdr p2)) (- (cdr p1) (cdr p2)))])
    (sqrt (+ t1 t2))))
...
(define (distance p1 p2)
  (let
      ([func1 (λ() 7)]
       [func2 (λ(x) p1)]
       [func3 (λ(x) p2)])
    (compute-length func1 func2 func3)))
...
(define (compute-f-with-step f a b step)
  (let calc
    ...
    (if(> index b) (reverse lst)
       (calc (cons (f index)  lst) (+ index step)))))  
...
(define (compute-perimeter points)
  (let perim
    ([total 0]
     [lst (reverse (cons (last points) points))])
    (if (= (length lst) 1) total
        (perim (+ total (distance (car lst) (cadr lst))) (cdr lst))))) 
...
(define (3-sequence-max numbers separator)
  (let maxsumF ([summ 0]
               ...
               [maxsum 0])
    (cond
      [(null? lst) (max summ maxsum)]
      [(= (car lst) separator) (maxsumF 0 (cdr lst) (max summ maxsum))]
      [else (maxsumF (+ (car lst) summ) (cdr lst) maxsum)]
      )))
